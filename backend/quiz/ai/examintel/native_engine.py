...
def extract_native_exam_questions(
    ...
    for l in raw_lines:
        if any(hdr in l for hdr in ["Oswaal SSC CGL", "SOLVED PAPER (12th September 2025", "Tier-I Year-wise"]):
            continue
        if l.isdigit() and int(l) <= 15:
            continue
        filtered_lines.append(l)

    # 3. Sequential Question State Machine
    questions_raw = []
    curr_q = None
    next_expected_q = 1
    ...
    for line in filtered_lines:
        ...
        is_true_q_start = False
        m_start = re.match(rf"^{next_expected_q}\.\s+(.*)", line)
        if m_start:
            content = m_start.group(1).strip()
            if re.search(r"\t\s*[1-4]\.\s+", content):
                is_true_q_start = False
            elif next_expected_q in [2, 3, 4] and curr_q and not any(re.search(r"^\s*4\.\s+", l) or "\t 4." in l for l in curr_q["raw_lines"]):
                if re.match(r"^(?:Select|In|What|Each|Which|Find|Rearrange|A\b|The\b|If\b|According|From|How|Why|Two|Three|Four|When|Who|Where|Study|Consider|Arrange|Given|Identify|Pointing|Introducing|Statements|Read|Choose|Fill|Complete|Below|Direction|Passage|Match|Solve|Calculate)\b", content):
                    is_true_q_start = True
                else:
                    is_true_q_start = False
            else:
                is_true_q_start = True
        ...
        if is_true_q_start:
            curr_q = {
                ...
                "sec": curr_sec,
                "first_line": m_start.group(1).strip(),
                "raw_lines": [],
            }
    ...
    # 4. Generate Individual 300 DPI Question Crops
    q_crop_map: Dict[str, str] = {}
    if output_dir and crops_dir:
        crop_expected_q = 1
        for p_idx in range(min(7, len(doc))):
            page = doc[p_idx]
            page_num = p_idx + 1
            ...
            for col_idx, col_lines in enumerate([col0_lines, col1_lines]):
                ...
                for l_idx, (bbox, txt) in enumerate(col_lines):
                    if "Answer Key" in txt and p_idx >= 5:
                        break
                    m = re.match(rf"^{crop_expected_q}\.[\t\s\u2000-\u200b]+(.*)", txt)
                    if m:
                        content = m.group(1).strip()
                        if re.search(r"\t\s*[1-4]\.\s+", content):
                            continue
                        if crop_expected_q in [2, 3, 4] and not re.match(r"^(?:Select|In|What|Each|Which|Find|Rearrange|A\b|The\b|If\b|According|From|How|Why|Two|Three|Four|When|Who|Where|Study|Consider|Arrange|Given|Identify|Pointing|Introducing|Statements|Read|Choose|Fill|Complete|Below|Direction|Passage|Match|Solve|Calculate)\b", content):
                            continue
    ...
    for q_data in questions_raw:
        ...
        for tok in opt_tokens:
            ...
            if m_opt:
                lbl = m_opt.group(1) or m_opt.group(2)
